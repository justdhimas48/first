...
class DatapelamarController extends Controller
{
   ...
   public function index(Request $request)
   {
      ...
      $tampil = DB::table('datapelamar')
         ...
         ->whereRaw('(datapelamar.pekerjaan_tujuan LIKE "%' . $request->search . '%" or datapelamar.jurusan_bidang LIKE "%' . $request->search . '%")')
         ->paginate(10);

      return view('datapelamar.index', compact('tampil'));
   }

   public function printPelamar(Request $request)
   {
      $tampil = DB::table('datapelamar')
         ->join('tbl_status_pelamar', 'datapelamar.status', '=', 'tbl_status_pelamar.id')
         ->select(DB::raw('datapelamar.*, tbl_status_pelamar.nama as nama_status'))
      ...
      return view('pdf.data-pelamar-print', compact('tampil'));
   }
   ...
   public function store(Request $request)
   {
      ...
      $this->validate($request, [
         ...
         // 'foto_diri' => 'required',
         // 'foto_skck' => 'required',
         // 'pekerjaan_tujuan' => 'required',
         // 'kompetensi_k' => 'required_if:pekerjaan_tujuan.*,in:dosen',
         'password' => 'required',
   ...
   public function printDataPelamar(Request $request)
   {
      // $data = Datapelamar::where('id', $request->id);
      $datapelamar = Datapelamar::where('id', $request->id)->first();
      // $data->user = User::find($request->id);

      // $hasilInterview = HasilInterview::where('id_pelamar', $request->id)->get();
      // $isLolos = true;
      ...
      return view('pdf.detail-pelamar-print', compact('datapelamar'));
   }
   ...
   public function cetakSK(Request $request, $id)
   {
      ...
      if ($request->jenis_pekerjaan == 'Pegawai') {
         ...
         $pegawai->nip = Datapegawai::generateNip();
         ...
         $pegawai->status = 1;
         $pegawai->jabatan = $pelamar->pekerjaan_tujuan;
         $pegawai->agama = $request->agama;
         $pegawai->no_hp = $pelamar->no_hp;
         $pegawai->pendidikan_s1 = $pelamar->pendidikan_s1;
         $pegawai->pendidikan_s2 = $pelamar->pendidikan_s2;
         $pegawai->pendidikan_s3 = $pelamar->pendidikan_s3;
         $pegawai->email = $request->email;
         $pegawai->tanggal_sk = date('Y-m-d');
         $pegawai->jurusan = $pelamar->jurusan_bidang;
         ...
         $unitkerja->nip = DataUnitKerja::generateNip();
         ...
         $unitkerja->pendidikan_s1 = $pelamar->pendidikan_s1;
         $unitkerja->pendidikan_s2 = $pelamar->pendidikan_s2;
         $unitkerja->pendidikan_s3 = $pelamar->pendidikan_s3;
   ...
   public function inputSK($id)
   {
      ...
      return view('datapelamar.input-sk', compact('pelamar', 'kategoriUnitKerja'));
   }
}
